...
def findx(t):
    if xmemo[t] != -1:
        ...
        xmemo[t] = findx((t-1))+0.05*((r/(1+u*findy(t-1)))*findx((t-1))-a*findx(t-1)*findy(t-1)+d*finda(t-1))
        return xmemo[t]
...
def findy(t):
    if ymemo[t] != -1:
        ...
        ymemo[t] = findy(t-1)+0.05*((s)*findy(t-1)+b*findy(t-1)*findx(t-1)+e*finda(t-1))
        return ymemo[t]
